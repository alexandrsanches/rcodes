...
ref <- read.xlsx("PanMacro_tabelas.xlsx", sheet = "Data")
...
dates <- seq(ano_i, ano_i + 1)

paises <- c("China", "Japan", "United States")
blocos <- c("Euro area", "World")
...
download.file(url_paises, destfile = "L:/Area Macro/Conjuntura/Panorama Macroeconomico/_temas_auto/Acessórios/dados_paises.csv")
dados_paises <- read.delim("L:/Area Macro/Conjuntura/Panorama Macroeconomico/_temas_auto/Acessórios/dados_paises.csv")
...
download.file(url_blocos, destfile = "L:/Area Macro/Conjuntura/Panorama Macroeconomico/_temas_auto/Acessórios/dados_blocos.csv")
dados_blocos <- read.delim("L:/Area Macro/Conjuntura/Panorama Macroeconomico/_temas_auto/Acessórios/dados_blocos.csv")

dados_paises <- dados_paises[,c(1:2,6:13)]
names(dados_paises) <- c("País", "Descrição", anos)
dados_paises <- subset(dados_paises, País %in% paises)

dados_blocos <- dados_blocos[,c(1:2,6:13)]
names(dados_blocos) <- names(dados_paises)
dados <- subset(dados_blocos, País %in% blocos)

for(i in c(3:10)){
    dados_blocos[,i] <- as.numeric(dados_blocos[,i])
    dados_paises[,i] <- as.numeric(dados_paises[,i])
}

for(i in c(1:2)){
    dados_blocos[,1] <- as.character(dados_blocos[,1])
    dados_paises[,1] <- as.character(dados_paises[,1])
}
...
write.xlsx(dados, file = "~/Alexandre/dados.xlsx")
