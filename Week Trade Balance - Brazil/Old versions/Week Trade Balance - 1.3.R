...
is.defined <- function(teste) {
  teste <- deparse(substitute(teste))
  env <- parent.frame()
  exists(teste, env)
}
...
date <- max(bc_ref[,2]) %>% floor_date("month")
...
semana_df <- data.frame('X1' = paste(semana,"? semana", sep = ""), 'X3' = index(calendario_semanal), 'X4' = weekdays(as.Date(index(calendario_semanal),'%d-%m-%Y'), abbreviate = T), 'X5' = last(bc$X4),'X6' = last(bc$X5), 'X7' = last(bc$X6), 
                     'X8' = last(bc$X7), 'X9' = last(bc$X8), 'X10' = last(bc$X9))

names(semana_df) <- names(bc_ref)

if(nrow(semana_df) == 2){
    if(bc_ref[nrow(bc_ref), 2] > semana_df[1, 2]){
	semana_df = semana_df[-1, ]
    }
}
